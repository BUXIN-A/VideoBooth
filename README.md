...
- 左下角预览框：显示画面全景，蓝色虚线框表示当前屏幕可见区域，可直接拖动虚线框平移画面；按住预览框的其他区域可拖动预览框本身（自动避开功能栏，位置仅本次运行有效）
...
- **设置面板**：直接在展台窗口内绘制（全屏窗口下独立窗口会被压在画面之下），分为「基础 / 画面 / 渲染 / 关于」四个选项卡；打开期间暂停采集，仅在面板需要重绘时出帧；内容超出窗口高度时可滚动
- **关于页**：显示程序图标、名称与版本，并提供「检查更新」——读取 GitHub 最新 release 与当前版本比较，发现新版本时弹出浮层询问是否前往下载，已是最新时仅在页内提示
...
│  ┕ UpdateChecker.cpp/.h   检查 GitHub 最新 release（后台 WinHTTP 请求）
...
- `guiScaleAuto`：GUI 大小是否随窗口自适应（默认 `true`）
- `guiScale`：`guiScaleAuto` 为 `false` 时使用的固定倍率，取值 0.75–2.0（默认 `1.0`）
...
| 检查更新 | WinHTTP 请求 GitHub Releases API（后台线程，单次超时 3 秒） |
...
- **界面自适应**：界面缩放 = DPI 缩放 ×（GUI 大小档位或按窗口尺寸自适应）；功能栏按钮总尺寸超出窗口时整体缩小，保证按钮始终可见
...
| `icon.png` | 设置「关于」页显示的程序图标 |
| `slidebutton.png` | 当前未使用 |
...
- 「双缓冲」与「抗锯齿」需重启程序生效
- 「GUI 大小」与「功能栏位置」保存后立即生效；自适应模式按窗口尺寸实时调整
- 预览框拖动后的位置仅在本次运行内有效，重启后回到默认位置
- 「检查更新」需要能访问 GitHub（无网络或网络受限时会提示检查失败），仅在你点击按钮时发起请求
